function initTareas(G) {
    ...
    (function() {
        ...
        const tabBtnCal     = document.getElementById('tab-btn-calendario');
        const tabBtnEvs     = document.getElementById('tab-btn-eventos');
        const tabLabelEvs   = document.getElementById('tab-label-eventos');
        const btnVolverCal  = document.getElementById('btn-volver-calendario');
        ...
        function cambiarVistaMobile(vista) {
            if (vista === 'eventos') {
                modal.classList.add('modo-eventos-mobile');
                if (tabBtnCal) tabBtnCal.classList.remove('activo');
                if (tabBtnEvs) tabBtnEvs.classList.add('activo');
                if (!diaActivo) {
                    const hoy = new Date();
                    diaActivo = toKey(hoy.getFullYear(), hoy.getMonth(), hoy.getDate());
                    renderCal();
                    renderDia();
                    const formEv = document.getElementById('form-evento');
                    if (formEv) formEv.style.display = 'flex';
                }
            } else {
                modal.classList.remove('modo-eventos-mobile');
                if (tabBtnCal) tabBtnCal.classList.add('activo');
                if (tabBtnEvs) tabBtnEvs.classList.remove('activo');
            }
        }
        ...
        function renderDia() {
            ...
            if (!diaActivo) {
                tit.textContent = 'Selecciona un dia';
                list.innerHTML = '';
                if (tabLabelEvs) tabLabelEvs.textContent = 'Tareas y Eventos';
                return;
            }
            const [y,m,d] = diaActivo.split('-');
            tit.textContent = `${parseInt(d)} de ${MESES[parseInt(m)-1]} ${y}`;
            if (tabLabelEvs) {
                tabLabelEvs.textContent = `${parseInt(d)} ${MESES[parseInt(m)-1].slice(0,3)}`;
            }
        ...
        document.getElementById('btn-guardar-evento').addEventListener('click', () => {
            ...
            if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
                ...
                if (Math.ceil((fecha - hoy) / 86400000) === 1) {
                    ...
                    if (delay > 0) {
                        setTimeout(() => {
                            if (typeof G.enviarNotificacion === 'function') {
                                G.enviarNotificacion('Recordatorio ASEAC', `Mañana: ${mat} - ${desc} a las ${hora}`);
                            }
                        }, delay);
                    }
                }
        ...
        if (tabBtnCal) {
            tabBtnCal.addEventListener('click', () => cambiarVistaMobile('calendario'));
        }
        if (tabBtnEvs) {
            tabBtnEvs.addEventListener('click', () => cambiarVistaMobile('eventos'));
        }
        if (btnVolverCal) {
            btnVolverCal.addEventListener('click', () => cambiarVistaMobile('calendario'));
        }
        if (btnIrHoy) {
            btnIrHoy.addEventListener('click', () => {
                const hoy = new Date();
                navY = hoy.getFullYear();
                navM = hoy.getMonth();
                diaActivo = toKey(navY, navM, hoy.getDate());
                renderCal();
                renderDia();
                const formEv = document.getElementById('form-evento');
                if (formEv) formEv.style.display = 'flex';
